...
then
  ...
  aws s3 cp s3://api-of-things-releases/api_of_things-$TAG.tar.gz _build/prod/
fi
...
scp _build/prod/api_of_things-$TAG.tar.gz $HOST:releases/
...
printcyan "symlinking api_of_things to $TAG on $HOST..."
ssh $HOST "ln -s releases/$TAG/ api_of_things"
...
ssh $HOST "rm releases/api_of_things-$TAG.tar.gz"
