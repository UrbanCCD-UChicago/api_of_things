defmodule Aot.Testing.M2MActionsTest do
  ...
  describe "create_network_node/1" do
    ...
    test "using NetworkQueries.include_nodes/1 returns the associated nodes", %{network: network, node: node} do
      {:ok, _} = M2MActions.create_network_node(network: network, node: node)

      network = NetworkActions.get!(network.id, include_nodes: true)
    ...
    test "using NodeQueries.include_networks/1 returns the associated networks", %{network: network, node: node} do
      {:ok, _} = M2MActions.create_network_node(network: network, node: node)

      node = NodeActions.get!(node.id, include_networks: true)
  ...
  describe "create_network_sensor/1" do
    ...
    test "using NetworkQueries.include_sensors/1 returns the associated sensors", %{network: network, sensor: sensor} do
      {:ok, _} = M2MActions.create_network_sensor(network: network, sensor: sensor)

      network = NetworkActions.get!(network.id, include_sensors: true)
    ...
    test "using SensorQueries.include_networks/1 returns the associated networks", %{network: network, sensor: sensor} do
      {:ok, _} = M2MActions.create_network_sensor(network: network, sensor: sensor)

      sensor = SensorActions.get!(sensor.id, include_networks: true)
  ...
  describe "create_node_sensor/1" do
    ...
    test "using NodeQueries.include_sensors/1 returns the associated sensors", %{node: node, sensor: sensor} do
      {:ok, _} = M2MActions.create_node_sensor(node: node, sensor: sensor)

      node = NodeActions.get!(node.id, include_sensors: true)
    ...
    test "using SensorQueries.include_nodes/1 returns the associated nodes", %{node: node, sensor: sensor} do
      {:ok, _} = M2MActions.create_node_sensor(node: node, sensor: sensor)

      sensor = SensorActions.get!(sensor.id, include_nodes: true)
